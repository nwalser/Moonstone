...
public class MutationStream<TModel> : BackgroundWorker<MutationStream<TModel>> where TModel : new()
{
    ...
    private readonly ConcurrentQueue<MutationEnvelope> _newMutations;
    ...
    public MutationStream(ConcurrentQueue<MutationEnvelope> newMutations, ConcurrentQueue<Snapshot<TModel>> newSnapshot, PathProvider paths, CancellationToken ct, ILogger<MutationStream<TModel>> logger, MutationHandler<TModel> handler) : base(ct, logger)
    {
    ...
    private async Task<bool> IngestNewMutations(CancellationToken ct)
    {
        ...
        MutationEnvelope? oldestMutation = default;
    ...
    private async Task RebuildSnapshots(CancellationToken ct)
    {
        ...
        foreach (var wantedSnapshotAge in WantedSnapshotAges.OrderDescending())
        {
            ...
            var remainingMutations = _store.CachedMutations
                ...
                .AsEnumerable()
                .Select(CachedMutation.ToMutationEnvelope)
                .ToList();
            ...
            foreach (var cachedMutation in remainingMutations)
                snapshot.AppendMutation(cachedMutation, _handler);
