...
using RT.Comb;
using IMutation = Moonstone.Workspace.Data.IMutation;
using Log = Serilog.Log;

namespace Moonstone.Workspace.Workspace;
...
public class Workspace<TProjection> where TProjection : new()
{
    ...
    private IMutationStream? _mutationWriter;
    private PostgreSqlCombProvider _comb;

    private readonly ConcurrentQueue<string> _changedFiles = new();
    private readonly ConcurrentQueue<MutationEnvelope> _mutationsToWrite = new();
    ...
    public Workspace(PathProvider paths, MutationHandler<TProjection> mutationHandler)
    {
        ...
        _comb = new PostgreSqlCombProvider(new SqlDateTimeStrategy());
    }
    ...
    public async Task Open(CancellationToken ct = default)
    {
        ...
        for (var i = 0; i < changedFiles.Count; i++)
        {
            ...
            var mutations = MutationLog.Read(changedFile, ct);
        ...
        _mutationWriter = RollingMutationStream.InitializeFrom(_paths.GetSessionMutationsFolder(), ct);
    ...
    public void ApplyMutation(IMutation mutation)
    {
        ...
        _mutationsToWrite.Enqueue(new MutationEnvelope()
        {
            Id = _comb.Create(),
            Mutation = mutation,
        });
    ...
    private async Task ProcessChangedFiles(CancellationToken ct = default)
    {
        ...
        while (_changedFiles.TryPeek(out var relativePath) && !ct.IsCancellationRequested)
        {
            ...
            var mutations  = MutationLog.Read(path, ct);
    ...
    private async Task ProcessMutationWrites(CancellationToken ct = default)
    {
        ...
        var mutationsToIngest = new List<MutationEnvelope>();
        while (_mutationsToWrite.TryPeek(out var mutation) && !ct.IsCancellationRequested)
        {
            _mutationWriter.Append(mutation, ct);
